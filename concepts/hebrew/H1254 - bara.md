---
strongs: H1254
hebrew: בָּרָא
transliteration: bara
pronunciation: bah-RAH
root: ברא
gloss: to create (divine creation ex nihilo)
pos: verb, Qal
semantic-domain:
  - "[[concepts/themes/Creation]]"
  - "[[concepts/themes/Theology Proper]]"
testament: OT
occurrences: 54
lxx-equivalent: "[[concepts/greek/G2936 - ktizo]]"
related:
  - "[[concepts/hebrew/H6213 - asah]]"
  - "[[concepts/hebrew/H3335 - yatsar]]"
  - "[[concepts/hebrew/H430 - Elohim]]"
tags:
  - concept/hebrew
  - strongs
---

# בָּרָא — bara

The Hebrew verb exclusively reserved for divine creative activity. Unlike [[concepts/hebrew/H6213 - asah|עָשָׂה]] (*asah*, "to make/do") or יָצַר (*yatsar*, "to form/fashion"), *bara* never takes a material object — God alone is its subject, and the act described is without analogy in human experience.

## Morphology

- **Root**: ברא
- **Stems used**: Qal (active), Niphal (passive — "to be created"), Piel (rare — "to cut down")
- **Key distinction**: Never used with a preposition indicating source material. God does not *bara* something "from" something else.

## Usage Pattern

| Verb | Subject | Material? | Meaning |
|:---|:---|:---:|:---|
| **בָּרָא** (*bara*) | God only | No | Create *ex nihilo* |
| עָשָׂה (*asah*) | God or humans | Sometimes | Make, do, produce |
| יָצַר (*yatsar*) | God or humans | Yes (clay, etc.) | Form, fashion, shape |

## Occurrences in Genesis 1

*Bara* appears at three critical points in the creation narrative — each marking a genuinely new category of existence:

1. **[[bibles/NASB-IL/Genesis/Genesis 1#v1|Gen 1:1]]** — Creation of matter itself: "God **created** the heavens and the earth"
2. **[[bibles/NASB-IL/Genesis/Genesis 1#v21|Gen 1:21]]** — Creation of animate life: "God **created** the great sea creatures and every living soul"
3. **[[bibles/NASB-IL/Genesis/Genesis 1#v27|Gen 1:27]]** — Creation of humanity in God's image: "God **created** man" (×3 in one verse)

The threefold *bara* in v27 is emphatic — the creation of image-bearers ([[concepts/hebrew/H6754 - tselem|tselem]]) is the climactic divine act.

## Key Passages

- [[bibles/NASB-IL/Genesis/Genesis 1#v1|Gen 1:1]] — First word of divine action in Scripture
- [[bibles/NASB-IL/Genesis/Genesis 1#v27|Gen 1:27]] — Threefold use for humanity
- [[bibles/NASB-IL/Isaiah/Isaiah 45#v18|Isa 45:18]] — "God who **created** the heavens… who formed the earth and made it"
- [[bibles/NASB-IL/Isaiah/Isaiah 43#v1|Isa 43:1]] — "He who **created** you, O Jacob"
- [[bibles/NASB-IL/Psalms/Psalms 51#v10|Ps 51:10]] — "**Create** in me a clean heart" — only spiritual renewal uses *bara*

## NT Connection

The LXX renders *bara* as κτίζω (*ktizo*). Paul's "new creation" (2 Cor 5:17, Gal 6:15) uses the same verb — placing spiritual rebirth on the same ontological level as Genesis 1.
